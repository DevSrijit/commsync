...
export function Sidebar() {
  ...
  const { emails, setActiveFilter, activeFilter, imapAccounts, groups } =
    useEmailStore();
  const { channels: realtimeChannels, syncDiscordChannels } = useRealtimeStore();
  ...
  const messengersCount = realtimeChannels.length;
                ...
                >
                  ...
                  <CollapsibleContent className="pt-1 pb-2 space-y-1">
                    <div className="pl-4 py-2">
                      <div
                        className="flex items-center justify-between rounded-md hover:bg-accent hover:text-accent-foreground cursor-pointer p-2"
                        onClick={() => {
                          setShowDiscordAccountDialog(true);
                        }}
                      >
                    ...
                    <div className="px-2 py-1">
                      {discordAccounts.length > 0 ? (
                        <div className="space-y-1">
                          {discordAccounts.map((account) => (
                            <DiscordAccountCard
                              key={account.id}
                              id={account.id}
                              username={account.username || "Discord User"}
                        ...
                        <div className="text-xs text-muted-foreground p-2">
                          No Discord accounts connected. Add one to get started.
                        </div>
                      )}
                    </div>
                          ...
                          onClick={async () => {
                            try {
                              ...
                              toast({
                                ...
                                description: "Your Discord messages are up to date",
                              });
                            } catch (error) {
                              console.error("Error syncing Discord:", error);
                              toast({
                                title: "Sync Failed",
                                description: "Could not sync Discord messages",
                                variant: "destructive",
                              });
                            }
                          }}
