...
interface BulkvsAccountCardProps {
    id: string;
    phoneNumber: string;
    label: string;
    lastSync: string;
...
export function BulkvsAccountCard({
    id,
    phoneNumber,
    label,
    lastSync,
}: BulkvsAccountCardProps) {
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error("Failed to sync BulkVS account");
            }

            toast({
                title: "Sync started",
                description: "Your BulkVS messages are being synced in the background.",
            });
            ...
            toast({
                title: "Sync failed",
                description: "Could not sync your BulkVS messages.",
                variant: "destructive",
            });
        } finally {
            setIsSyncing(false);
    ...
    const handleDelete = async () => {
        setIsDeleting(true);
        try {
            const response = await fetch(`/api/bulkvs/account?id=${id}`, {
                method: "DELETE",
            });

            if (!response.ok) {
                throw new Error("Failed to delete BulkVS account");
            }

            toast({
                title: "Account deleted",
                description: "Your BulkVS account has been removed.",
            });
            router.refresh();
        } catch (error) {
            ...
            toast({
                ...
                description: "Could not delete your BulkVS account.",
                variant: "destructive",
            });
        } finally {
            setIsDeleting(false);
            ...
            <Card className="mb-4">
                <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                        ...
                        <Badge variant="outline">BulkVS</Badge>
                    </div>
            ...
            <AlertDialog open={showDeleteAlert} onOpenChange={setShowDeleteAlert}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will permanently delete this BulkVS account and remove all
