    ...
    try {
      ...
      if (!testResponse.ok) {
        ...
        if (statusCode === 401) {
          ...
          errorMessage =
            "JustCall server error. Please try again later or contact JustCall support.";
        } else if (errorData.message) {
          errorMessage = `JustCall API error: ${errorData.message}`;
        }

        console.error("JustCall credential validation failed:", {
          statusCode,
          error: errorData,
        });
      ...
      const phoneNumbersResponse = await fetch(
        "https://api.justcall.io/v2.1/phone-numbers",
        {
          method: "GET",
          headers: {
            Authorization: `Basic ${basicAuth}`,
      ...
      if (!phoneNumbersResponse.ok) {
        ...
        return NextResponse.json(
          {
            error:
              "Could not verify JustCall phone numbers access. API key may have limited permissions.",
          },
          { status: 400 }
        );
      }
      ...
      if (!smsData || typeof smsData.data === "undefined") {
        return NextResponse.json(
          { error: "Invalid response from JustCall API" },
          { status: 400 }
        );
      }
      ...
      console.log(
        "JustCall API raw response:",
        JSON.stringify(phoneNumbersData, null, 2).substring(0, 500) + "..."
      );
      console.log(
        "JustCall Available Phone Numbers:",
        phoneNumbers.map((entry: any) => entry.justcall_number || "N/A")
      );
      console.log(
      ...
      const phoneNumberExists = phoneNumbers.some((entry: any) => {
        ...
        const normalizedJustCallNumber = (entry.justcall_number || "").replace(
          /\D/g,
          ""
        );
        ...
        return (
          entry.justcall_number === phoneNumber ||
          ...
          (normalizedInputNumber.length > 10 &&
            normalizedJustCallNumber.endsWith(
              normalizedInputNumber.slice(-10)
            )) ||
          (normalizedJustCallNumber.length > 10 &&
            normalizedInputNumber.endsWith(normalizedJustCallNumber.slice(-10)))
        );
      });
      ...
      if (!phoneNumberExists && phoneNumbers.length > 0) {
        return NextResponse.json(
          {
            error:
              "The provided phone number does not exist in your JustCall account",
            availableNumbers: phoneNumbers.map(
              (entry: any) => entry.justcall_number || entry.friendly_number
            ),
            providedNumber: phoneNumber,
            normalizedNumber: normalizedInputNumber,
            message:
              "Please use one of the available phone numbers from your JustCall account. Make sure to use the exact format shown.",
          },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json(
        {
          error:
            "Could not validate JustCall credentials. Please check your network connection and try again.",
        },
        { status: 400 }
      );
    }
