...
import { syncAllAccountsForUser, syncJustCallAccounts } from '@/lib/sync-service';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to run global sync', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const syncPromise = options.platform === 'justcall'
      ? syncJustCallAccounts(userId)
