...
// Ensure stripe instance is available
if (!stripeClient) {
  throw new Error("Stripe client is not properly initialized");
}

// Use a properly typed stripe instance
const stripe = stripeClient as Stripe;
...
export async function POST(
  ...
  try {
    ...
    if (!organization) {
      ...
      logger.info("Updating organization with missing Stripe customer ID", {
        orgId: organization.id,
      });
